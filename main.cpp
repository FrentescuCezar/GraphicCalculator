...
//Rezolutia adaptabila a ecranului , Fullscreen default .
...
//Functia introdusa de la tastatura
char s[MAX];


//Variabile pentru intervalele graficului
int a=-10,b=10;
int n=0, i;
...
int main()
{
    ...
    do
    {
        ...
        if(Laborator==1)
        {
            //Citim functia de la tastatura
            Citeste_Functie(s, lungime/2.32, latime/3.8, lungime, latime, errorf, ErrorType);
            Laborator=1;

            //Verificam corectitudinea ei, daca exista o eroare nu se va afisa graficul
            if(!scrierecorecta(s,errorf,ErrorType))
            {
                ...
                //Animatia monitorului, se va afisa doar prima oara de la deschiderea programului
                if(AnimatieMonitor_Played==0)
                {
                    Animatie_Monitor(Laborator, lungime, latime);
                    delay(800);
                ...
                //Verificam daca functia respectiva este o constanta.
                int cok=0,cnr=0;
                bool cgas=0;
                ...
                //Gasim asimptotele, desenam graficul, afisam butoanele,apoi asimptotele, punctele de minim si de maxim in caz de acestea sunt ON.
                ...
                while(ButonZoom!=3)
                {
                    ...
                    if(AsimptotaButonApasat == 1) // Butonul de Asimptote
                    {
                    ...
                    if(MaxButonApasat == 1) // Butonul de Max
                    {
                    ...
                    if(MinButonApasat == 1) // Butonul de Min
                    {
...
void CLICK(int x, int y)
{
    ...
    {
        //Butoanele se afla in afisare_menu.h
        //Coordonatele butoanelor in calculeaza_coordonate.h
        ...
        {
            if ( x >= coordBack.x1 && x <= coordBack.x2 && y >= coordBack.y1 && y <= coordBack.y2 ) // BUTONUL DE BACK
            {
                Back();
                ButonZoom = 3;
                Laborator=0;
            ...
            else if(Buton == INCEPE) // Ecranul laboratorului
            {
            ...
            else if (Buton == FUNCTIE) // FUNCTIA GRAFICULUI
            {
                ...
                if (x >= lungime-lungime/12 && x <= lungime-lungime/24 && y >= latime-latime/15 && y <= latime) // Butonul de Zoom Out  ( - )
                {
                    ButonZoom = 2;
                }

                if (x >= lungime-lungime/40 && x <= lungime && y >= latime-latime/15 && y <= latime) // Butonul de Zoom In ( + )
                {
                    ButonZoom = 1;
                }

                if (x >= 10 && x <= lungime-lungime/1.13 && y >= latime-latime/13.9 && y <= latime-10) // Butonul de Asimptota
                {
                ...
                if (x >= lungime-lungime/1.21 && x <= lungime-lungime/1.3 && y >= latime-latime/13.9  && y <= latime-10) // Butonul de Min
                {
                    MinButonApasat=1;
                ...
                if (x >= lungime-lungime/1.13 && x <= lungime-lungime/1.2 && y >= latime-latime/13.9 && y <= latime-10) // Butonul de Max
                {
                    MaxButonApasat=1;
            ...
            else if (Buton == LIMBA) // MENU LANGUAGE
            {
                ...
                if(x >= coordLimba_Romana.x1 && x <= coordLimba_Romana.x2 && y >= coordLimba_Romana.y1 && y <= coordLimba_Romana.y2) // OMULET ROMANA
                {
                    Limba = RO;
                    readimagefile("Jpg/Romana/Limba_Romana.jpg", 0, 0, maxx, maxy);
                }

                if(x >= coordLimba_Franceza.x1 && x <= coordLimba_Franceza.x2 && y >= coordLimba_Franceza.y1 && y <= coordLimba_Franceza.y2)// OMULET FRANCEZA
                {
                    Limba = FR;
                    readimagefile("Jpg/Franceza/Limba_Franceza.jpg", 0, 0, maxx, maxy);
                }

                if(x >= coordLimba_Engleza.x1 && x <= coordLimba_Engleza.x2 && y >= coordLimba_Engleza.y1 && y <= coordLimba_Engleza.y2) // OMULET ENGLEZA
                {
                    Limba = ENG;
